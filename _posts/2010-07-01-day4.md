...
* Guys, start using [http://ask.scipy.org](http://ask.scipy.org) for your scientific computing questions, particularly with python! Enthought and the scipy people really want this to reach critical mass. The reason for this and not StackOverflow is to avoid the noise that is generated by the more mainstream python community: for example, searching "optimization" would come out actually showing something on finding minimums instead of the more common code optimization.
...
* Josh Hemann wants to collect publication-quality, full-featured examples of matplotlib that show more than just, "look you can totally make a pie graph! Foo, bar." Basically, he wants to [best R's examples](http://addictedtor.free.fr/graphiques/thumbs.php). If you have anything, you can email him at josh.hemannATroguewave.com for now.

* [Creative Digital Systems](http://creativedigitalsys.com/) made a pretty sweet radar simulator. Unfortunately, the projector was too jacked up for us to demo the thing.
